...
############################################
## NEW SECTION TO STORE RESULTS BACKWARDS ##
############################################

        if os.path.exists("mod_results.json"):
            f=open("mod_results.json", 'r')
            mod_dict = json.load(f)
            f.close()
        else:
            mod_dict = {}
        desc_words=item_desc.split(" ")
        jewel_number=0
        for word in desc_words:
            if not word.isalpha():
                jewel_number=int(word)
        for item_dict in item_stats:
            all_nodes=item_dict["socket_nodes"]
            for a_node in all_nodes:
                this_mod=a_node["mods"]
                if len(this_mod)>1:
                    continue
                if this_mod[0] not in mod_dict.keys():
                    current_list=[]
                else:
                    current_list=mod_dict[this_mod[0]]
                if [jewel_number,a_node["id"]] not in current_list:
                    new_list=current_list+[[jewel_number,a_node["id"]]]
                    mod_dict[this_mod[0]]=new_list

        #for key in mod_dict:
        #    print(key,mod_dict[key])
        #pickle the mod_dict
        f=open("mod_results.json", 'w')
...
############################################
    ...
    def _tree_to_screen(self, tree_coords):
        ...
        screen_coords=(int(tree_coords[0]*self.t2s_scale),int(tree_coords[1]*self.t2s_scale))
    ...
    def _move_to_tree_pos_using_spaces(self, tree_desired_pos, max_position_error=2):
        ...
        target = (int(tree_desired_pos[0]*self.t2s_scale),int(tree_desired_pos[1]*self.t2s_scale))
#we only check the x bound, since all nodes are in bounds in the y direction
        ...
        while (abs(dx) + abs(dy)) > max_position_error:
            ...
            screenshot = grab_screen(tuple([760,340,1160,740]))
            img= Image.fromarray(screenshot)
            img.save("screenshot"+str(self.camera_position[0])+","+str(self.camera_position[1])+".png")
            self.log.info("Moved, took screenshot at %d %d",self.camera_position[0],self.camera_position[1])
    ...
    def _locate_screen(self, quadrent):
        ...
        img= Image.fromarray(screenshot)
        img.save("screenshot"+str(self.camera_position[0])+","+str(self.camera_position[1])+".png")
        self.log.info("Got Bottom Left, Took screenshot at %d %d",self.camera_position[0],self.camera_position[1])
    ...
    def _analyze_nodes(self, socket_id, repeated=False):
        ...
        tree_socket_pos=node_coords[str(socket_id)]

        screenshot = grab_screen(tuple([760,340,1160,740]))
        img= Image.fromarray(screenshot)
        img.save("screenshot"+str(self.camera_position[0])+","+str(self.camera_position[1])+".png")
        self.log.info("Analyzing a node, took screenshot at %d %d",self.camera_position[0],self.camera_position[1])
...
#REMOVE
        ...
        self.camera_position[0] -= offset[0]
        self.camera_position[1] -= offset[1]
        return nodes
    ...
    def _get_node_data(self, node_id,offset):
        ...
        location = self._tree_to_screen(node_coords[str(node_id)])
        ...
        for node in nodes_lines:
            ...
            for line in node["stats"]:
                ...
                if filtered_line in self.passive_names:
                    ...
                    #self.log.info("Triggered fallback")
                    # actually just skip it. No false positives is important.
                    continue
