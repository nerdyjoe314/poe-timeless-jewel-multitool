...
non_skills=['ascendancyName','Small Jewel Socket','Medium Jewel Socket','isBlighted','isMastery','isProxy','classStart','isKeystone']
while current_line<total_lines:
	if lined_input[current_line][:10]==next_node_trigger:
		...
		while i<100:
			...
			if not is_skill:
				break
			if text_in_line(current_line+i,notable_flag):
				is_notable=True
			if text_in_line(current_line+i,group_str):
				...
				# skip the bulk of this node text
				current_line+=i
				break
			i+=1
		if is_notable:
			notables_list.append(node)
		else:
			regulars_list.append(node)
	current_line+=1
...
#jewel socket above witch is 61419
#a minion life node not in radius is 32024
#j=node_coords[61419]
#outside=node_coords[32024]
#print(math.sqrt((j[0]-outside[0])**2+(j[1]-outside[1])**2))
...
### make a dictionary with whether each node is notable or not
node_types={}
for n in node_list:
	if n[0] in notables_list:
		node_types[n[0]]="notable"
...
f.close()
f = open("neighbor_nodes.json", 'w')
json.dump(neighbor_nodes,f)
f.close()
f = open("node_types.json", 'w')
